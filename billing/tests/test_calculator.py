...
from billing.core.types import ChargeList, CustomerCharge, DemandCharge, EnergyCharge, PeakType
...
def simple_customer_charge():
    ...
    return CustomerCharge(
        name="Test Customer",
        amount_usd_per_month=Decimal("25.00"),
    )
...
def test_apply_charges_empty_charges(hourly_day_usage):
    """
    Empty ChargeList returns usage DataFrame unchanged.

    Expected: No charge columns added, only original usage columns
    """
    empty_charges = ChargeList(
...
def test_calculate_monthly_bills_single_month(hourly_day_usage, simple_energy_charge):
    """
    Single month of usage creates one MonthlyBillResult.
    ...
    result = results[0]
    assert result.month_start == date(2024, 1, 1)
    assert result.month_end == date(2024, 1, 31)
...
def test_calculate_monthly_bills_multi_month(usage_df_factory, simple_energy_charge):
    ...
    # Results should be sorted by month_start
    assert results[0].month_start == date(2024, 1, 1)
    assert results[1].month_start == date(2024, 2, 1)
    assert results[2].month_start == date(2024, 3, 1)
...
def test_calculate_monthly_bills_month_boundary_splitting(usage_df_factory, simple_energy_charge):
    ...
    assert results[1].month_start == date(2024, 2, 1)
    assert results[1].month_end == date(2024, 2, 29)  # 2024 is a leap year
...
def test_calculate_monthly_bills_empty_charges(hourly_day_usage):
    """
    Empty ChargeList creates valid result with no line items.
